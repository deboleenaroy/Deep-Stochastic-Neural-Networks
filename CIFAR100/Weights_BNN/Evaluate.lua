...
cmd:option('-network',            'VGG9_unipolar_stochastic_sigmoid.lua',            'Model file - must return valid network.')
...
cmd:option('-load',               './Results/VGG9_unipolar_stochastic_sigmoid/model_50.t7',                     'load existing net weights')
--cmd:option('-save',               os.date():gsub(' ',''), 'save directory')
cmd:option('-save',               'ResNet20_Sigmoid_typeA_only_identity/test/', 'save directory')
...
------------------------------
local function Forward_TestOnly(Data)
  ...
  while NumSamples < SizeData do
    ...
    print('block 2 conv weight', model.modules[5].weight[1][1][1])
    print('block 2 bnorm output',  model.modules[7].output[1][1][1])
    print('block 2 sigmoid output', model.modules[8].output[1][1][1])
    print('block 2 stochastic output', model.modules[9].output[1][1][1])
    exit()
...
--Train
confusion:zero()
local LossTrain = Test(data.TrainData)
