...
cmd:option('-network',            'VGG16_stochSigmoid',            'Model file - must return valid network.')
cmd:option('-LR',                 0.005,                   'learning rate')
cmd:option('-LRDecay',            0,                      'learning rate decay (in # samples)')
cmd:option('-weightDecay',        0.0,                    'L2 penalty on the weights')
cmd:option('-momentum',           0.9,                    'momentum')
cmd:option('-dampening',            0,                    'dampening')
cmd:option('-batchSize',          50,                    'batch size')
cmd:option('-weightClamp',        true,                 'clamp weights')
cmd:option('-stcNeurons',         false,                   'use stochastic binarization for the neurons')
...
cmd:option('-epoch',              400,                     'number of epochs to train, -1 for unbounded')
...
cmd:option('-save',               'VGG16_detSigmoid_clampWeights', 'save directory')

cmd:text('===>Data Options')
cmd:option('-dataset',            'Cifar100',              'Dataset - Cifar10, Cifar100, STL10, SVHN, MNIST')
...
cmd:option('-preProcDir',         '/data/roy77/stochastic_NN/PreProcData2/',       'Data for pre-processing (means,P,invP)')
cmd:text('===>Misc')
...
local function Forward(Data, train)
  ...
  while NumSamples < SizeData do
    ...
    if train then
      ...
      if opt.weightClamp then
        --print('clamping weights')
      	Weights[clipV:eq(1)]=Weights[clipV:eq(1)]:clamp(-1,1)
      end
    end

    --print(model.modules[7].output[1][1])
...
while epoch <= opt.epoch do
    ...
    --print('weight before training', model.modules[1].weight[1][1])
    ...
    --print('trained weight', model.modules[1].weight[1][1])
