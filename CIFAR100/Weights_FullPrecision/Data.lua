...
if Dataset =='Cifar100' then
  ...
  if (paths.filep(file_valid) and paths.filep(file_train) and paths.filep(file_test)) then
    ...
    ds = dp.Cifar100{scale={0,1}, valid_ratio=0.01,input_preprocess = input_preprocess}
  ...
  print ('PreProcessed Data Load complete')
...
elseif Dataset == 'Cifar10' then
    ...
    if (paths.filep(file_valid) and paths.filep(file_train) and paths.filep(file_test)) then
      ...
      table.insert(input_preprocess, dp.ZCA())
      print ('PreProcessing Data')
      ds = dp.Cifar10{scale={0,1},valid_ratio=0.01,input_preprocess = input_preprocess} --,input_preprocess = input_preprocess}  scale={0,1},
...
--Preprocesss
  print("More preprocessing steps")
  if format == 'yuv' then
    require 'image'
