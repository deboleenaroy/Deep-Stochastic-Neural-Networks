...
local function paramsForEpoch(epoch)
    ...
    --    {   1, 100,     5e-3, 5e-4 },
    --    { 101, 200,   2.5e-3, 5e-4 },
    --    { 201, 300,  1.25e-3, 5e-4 },
    --    { 301, 400,  6.25e-4, 5e-4 },
...
function train()
   ...
   if newRegime then
      ...
      optimState.momentum = 0.9 --opt.momentum
   ...
   print(string.format('Epoch: [%d][TRAINING SUMMARY]'
                          ...
                          .. 'accuracy(%%):\t top-1 %.2f\t',
                       epoch, loss_epoch, top1Sum/opt.epochSize))
   print('\n')
   ...
   return top1Sum/opt.epochSize, loss_epoch
...
function trainBatch(inputsCPU, labelsCPU)
   ...
   labels:resize(labelsCPU:size()):copy(labelsCPU)
    ...
    --print(model.modules[4].modules[1].output[1][1][1])
    --print(model.modules[4].modules[2].output[1][1][1])
    --print(model.modules[4].modules[3].weight[1][1])
  ...
  local feval = function()
   ...
   one_hot_yt=torch.zeros(labels:size(1),opt.nClasses)
   one_hot_yt:scatter(2, labels:long():view(-1,1), 1)
   ...
   xlua.progress(batchNumber, 50000)
   --dataTimer:reset()
   --timer:reset()
end
