...
local trainHook = function(self, path)
   ...
   --roy77--local out = image.crop(input, w1, h1, w1 + oW, h1 + oH)
   --roy77--assert(out:size(3) == oW)
   --roy77--assert(out:size(2) == oH)
   out = input --roy77--
...
testHook = function(self, path)
   ...
   --roy77--local out = image.crop(input, w1, h1, w1+oW, h1+oH) -- center patch
   out = input --roy77--
   -- mean/std
   for i=1,3 do -- channels
      if mean then out[{{i},{},{}}]:add(-mean[i]) end
